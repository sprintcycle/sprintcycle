...
- **治理 / SDD / v7「二」补齐**：``[governance] packs`` 与主 ``config_path`` **顺序合并**（``yaml_merge``）；Planning 门 **SDD**（``spec_marker``、``acceptance_glob``、内存 ``ReleasePlan`` + ``ReleasePlanValidator``、backlog **``spec_ref``** 文件检查，``sprintcycle.governance.sdd_checks``）；Review 可选 **Compose 供应链提示**（``compose_supply_chain``，``:latest`` / Dockerfile）；``[execution] incremental_test_command`` 与 ``[governance] ci_matrix_tags`` 写入 **``run_metadata``** 并参与 **``measurement_context_hash``**；**``prompt_sources``** 扩展 Architect / Tester / RegressionTester；``run_planning_gate_sync(..., extra_context=…)`` 供测试与 CLI 注入上下文；文档与 Issue 表见 ``docs/GOVERNANCE_ENGINEERING.md`` §4 / §5 / §13 / §14
