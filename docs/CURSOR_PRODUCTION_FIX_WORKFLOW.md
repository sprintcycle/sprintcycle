...
- `.cursor/commands/ci-fix-loop.md` 是总流程，负责把“检测 → 修复 → 再检测”串起来。
- `/fix-python-imports` 和 `/fix-arch-imports` 是两个专用修复协议，一个处理路径迁移，一个处理分层边界。
...
| 簇 | 现象 | 处理 |
|----|------|------|
| A Import/路径 | `ModuleNotFoundError`、`ImportError`、`pytest --collect-only` 失败 | `/fix-python-imports` |
| B 架构契约 | `lint-imports` forbidden layer | `/fix-arch-imports` |
...
当失败来自分层违规时，使用 `/fix-arch-imports`。
