...
> **Canonical path (真源路径)**: `docs/SPRINTCYCLE_CONSTITUTION.md`  
> Former location `.specify/memory/constitution.md` retired with speckit (2026-06-18).  
> 原 `.specify/memory/constitution.md` 随 speckit 退役（2026-06-18）。
...
requirements.（理由：SprintCycle 是为闭环开发而设计的，因此可见性与恢复不是可选诊断能力，而是一等公民级的产品要求。）
...
2. **Grade and document** before coding: apply `docs/SPRINT_SDD_GATES.md` (L/F
   grade, impact surface, initial vs suggested scope). L2+ / F1+ work MUST have
   PRD and/or plan under `docs/sdd-designs/YYYY-MM-DD/` (template:
   `docs/templates/sdd-feature-template.md`). Medium+ refactors use
   `/sprint optimize`.（编码前分级与落档：见 SDD gates；L2+/F1+ 须 dated 文档。）
3. **HITL and principle review** before implementation: four-principle review
   table per SDD gates §6; optimize workflow requires PRD + technical plan
   approval.（实施前 HITL 与原则审查。）
4. Implement the smallest coherent change that satisfies the approved plan.（以满足已批准方案为目标，实现最小且完整的一组变更。）
5. Add or update tests for contract changes, orchestration changes, or
   governance-sensitive behavior.（为契约变更、编排变更或对治理敏感的行为补充或更新测试。）
6. Run linting, validation, and any required integration or governance checks.（运行 lint、验证以及所需的集成或治理检查。）
7. Review the change for observability, recovery, and promotion impact before
   merging or releasing; output closure one-line principle conclusion.（合并或发布前审查可观测性、恢复与晋升影响；输出一行原则结论。）
...
- If a change introduces new ambiguity in the lifecycle, the ambiguity MUST be
  resolved in the dated SDD document before implementation continues.（若变更引入生命周期歧义，须先在 dated SDD 文档中消除后再继续实现。）

**Deprecated (2026-06-18)**: GitHub spec-kit / `@speckit` / `.specify/` / top-level
`specs/` — historical specs archived under `docs/archive/specs/`.

## Governance（治理）

This constitution overrides lower-level conventions when they conflict.（当本宪法与更低层级的约定冲突时，本宪法优先。）
Amendments require a written rationale, an impact review across related design
documents, and an updated version number using semantic versioning:（修订需要书面理由、对相关设计文档的影响评审，以及按照语义化版本号更新版本号：）
...
**Version**: 1.1.0 | **Ratified**: 2026-05-17 | **Last Amended**: 2026-06-18

**Amendment 1.1.0 (2026-06-18)**: Retired speckit/spec-kit workflow; unified SDD
under `docs/SPRINT_SDD_GATES.md` and `docs/sdd-designs/`; constitution moved
from `.specify/memory/` to `docs/`.
