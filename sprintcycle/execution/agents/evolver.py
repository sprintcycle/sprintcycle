...
class EvolutionPath(Enum):
    """Agent 侧进化路径（按优化维度区分；V4.0 §6.7 / 与 ``strategies.EvolutionStrategy`` 执行类脱钩）。"""
...
EvolutionStrategy = EvolutionPath  # 后向兼容别名，新代码请用 EvolutionPath
...
class EvolverAgent(AgentExecutor):
    ...
    def _find_improvements(self, analysis: Dict[str, Any], context: AgentContext) -> List[Dict[str, Any]]:
        ...
        improvements.sort(key=lambda x: float(str(x["priority"])))
        return improvements
...
__all__ = ["EvolverAgent", "EvolutionPath", "EvolutionStrategy"]
