...
class QualityLifecycleHooks:
    ...
    async def on_before_task(self, context: QualityContext) -> bool:
        await self._maybe_hitl(context, gate="spec_confirm", title="Spec 确认", summary="AI 生成的结构化规范需要人工确认后继续")
    ...
    async def on_after_task(self, context: QualityContext) -> Report:
        await self._maybe_hitl(context, gate="execution_approval", title="任务执行确认", summary="任务完成后需要人工确认是否继续")
        return await self._full_check(context)

    async def on_before_release(self, context: QualityContext) -> Report:
        await self._maybe_hitl(context, gate="release_approval", title="发布前确认", summary="Release 前需要人工确认是否继续")
        return await self._full_check(context)

    async def on_after_release(self, context: QualityContext) -> Report:
        await self._maybe_hitl(context, gate="after_sprint", title="Sprint 结束确认", summary="Sprint 结束后需要人工确认结果")
        return await self._full_check(context)
    ...
    async def _maybe_hitl(self, context: QualityContext, *, gate: str, title: str, summary: str) -> Optional[str]:
        ...
        try:
            ...
            return None
        policy = evaluate_hitl_policy(gate=gate, context={"project_path": project_path, "summary": summary, **extra}, config=runtime_config)
