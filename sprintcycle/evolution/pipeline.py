...
- ``SprintResult`` / ``TaskResult``（``execution.sprint_types``）：**SprintOrchestrator / SprintExecutor**
  对真实 Sprint Backlog 的执行产物（含 ``work_item: SprintBacklogItem``、``ExecutionStatus``）。
...
若委托编排器，应经 ``release_plan_adapter.evolution_release_plan_to_release_plan`` 再进主线，UI 展示执行层结果请用 ``SprintResult`` / ``RunResult``。
"""
...
class EvolutionPipeline:
    ...
    def __init__(
        ...
        self._plan_source = plan_source or ManualReleasePlanSource()
    ...
    async def evolve_sprint(self, *, sprint: "SprintDefinition", max_generations: int = 3) -> Any:
