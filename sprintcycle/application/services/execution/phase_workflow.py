...
from sprintcycle.application.services.lifecycle.lifecycle_contracts import LifecycleContract, build_lifecycle_state_machine
...
def build_plan_artifact(
    ...
    *,
    ...
    evidence.setdefault("stages", {}).setdefault("plan", {}).update(
        {
...
def build_prepare_artifact(
    ...
    prepared["validation_refs"] = {
        **dict(prepared.get("validation_refs") or {}),
        **artifact.checks,
        "ready": artifact.ready,
        "blockers": artifact.blockers,
    }
    evidence = dict(prepared.get("evidence") or {})
    evidence.setdefault("stages", {}).setdefault("prepare", {}).update(
        {"ready": artifact.ready, "checks": artifact.checks, "blockers": artifact.blockers, "present": True}
    )
    prepared["evidence"] = evidence
    return {
        "lifecycle_contract": {
            **prepared,
            "plan_refs": dict(prepared.get("plan_refs") or {}),
            "validation_refs": prepared.get("validation_refs", {}),
            "evidence": evidence,
        },
        "prepare": artifact.to_dict(),
    }
...
def build_decompose_artifact(
    ...
    decomposed = machine.transition(
        ...
        "decomposed",
        status="success",
        reason="decomposition completed",
    ...
    evidence.setdefault("stages", {}).setdefault("decompose", {}).update(
        {"subtasks": subtasks, "dag": dag, "acceptance_criteria": artifact.acceptance_criteria, "present": True}
    )
...
def build_observe_artifact(
    ...
    *,
    ...
    artifact = ObserveArtifact(
        execution_id=str(observed.get("execution_id") or ""),
        task_id=str(observed.get("task_id") or ""),
        project_path=str(observed.get("project_path") or ""),
        trace=dict(trace or {}),
    ...
    evidence.setdefault("stages", {}).setdefault("observe", {}).update(
        {"trace": artifact.trace, "diagnostics": artifact.diagnostics, "present": True}
    )
    observed["evidence"] = evidence
    return {"lifecycle_contract": observed, "observe": artifact.to_dict()}


def build_diagnose_artifact(
    contract_payload: Dict[str, Any],
    *,
    ...
    artifact = DiagnoseArtifact(
        execution_id=str(diagnosed.get("execution_id") or ""),
        task_id=str(diagnosed.get("task_id") or ""),
        project_path=str(diagnosed.get("project_path") or ""),
    ...
    diagnosed["repair_refs"] = {
        **dict(diagnosed.get("repair_refs") or {}),
        "root_causes": artifact.root_causes,
        "repair_ready": artifact.repair_ready,
        "confidence": artifact.confidence,
    }
    evidence = dict(diagnosed.get("evidence") or {})
    evidence.setdefault("stages", {}).setdefault("diagnose", {}).update(
        {
            "root_causes": artifact.root_causes,
            "repair_ready": artifact.repair_ready,
            "confidence": artifact.confidence,
...
def build_repair_artifact(
    contract_payload: Dict[str, Any], *, closed_loop: bool = False, verify_result: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    repaired = dict(contract_payload or {})
    artifact = RepairArtifact(
        execution_id=str(repaired.get("execution_id") or ""),
        task_id=str(repaired.get("task_id") or ""),
        project_path=str(repaired.get("project_path") or ""),
        attempted=True,
        closed_loop=closed_loop,
        verify_result=dict(verify_result or {}),
        metadata={"source": "phase_workflow"},
    )
    repaired["repair_refs"] = {
        **dict(repaired.get("repair_refs") or {}),
        "closed_loop": artifact.closed_loop,
        "verify_result": artifact.verify_result,
    }
    evidence = dict(repaired.get("evidence") or {})
    evidence.setdefault("stages", {}).setdefault("repair", {}).update(
        {
    ...
    evidence.setdefault("stages", {}).setdefault("verify", {}).update(
        {"closed_loop": artifact.closed_loop, "verify_result": artifact.verify_result, "present": True}
    )
    repaired["evidence"] = evidence
    return {"lifecycle_contract": repaired, "repair": artifact.to_dict()}


def build_deliver_artifact(
    contract_payload: Dict[str, Any],
    *,
    ...
    artifact = DeliverArtifact(
        execution_id=str(delivered.get("execution_id") or ""),
        task_id=str(delivered.get("task_id") or ""),
        project_path=str(delivered.get("project_path") or ""),
    ...
    evidence.setdefault("stages", {}).setdefault("deliver", {}).update(
        {"outputs": artifact.outputs, "runtime_linkage": artifact.runtime_linkage, "present": True}
    )
    evidence.setdefault("runtime", {}).update(
        {"linked": bool(artifact.runtime_linkage), "healthy": bool(artifact.runtime_linkage)}
    )
