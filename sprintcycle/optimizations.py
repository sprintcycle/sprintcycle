...
class RollbackManager:
    ...
    def _cleanup_if_needed(self):
        ...
        if len(self.backup_history) > self.max_backups:
            self.cleanup_old_backups(self.max_backups)
    ...
    def get_backup_diff(self, backup_id: str, file_path: str) -> Dict:
        ...
        diff = list(difflib.unified_diff(
            ...
            fromfile="backup/" + file_path,
            tofile="current/" + file_path
        ))
    ...
    def begin_transaction(self, task_id: str, files: List[str]) -> str:
        """v4.10: 开始事务性备份"""
        backup_id = "txn_" + task_id + "_" + datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_subdir = self.backup_dir / backup_id
        backup_subdir.mkdir(parents=True, exist_ok=True)
    ...
    def rollback_transaction(self, backup_id: str = None) -> Dict:
        """v4.10: 回滚事务"""
        backup_id = backup_id or self._current_backup_id
        if not backup_id:
    ...
    def commit_transaction(self, backup_id: str = None) -> Dict:
        ...
        backup_id = backup_id or self._current_backup_id
        if not backup_id:
            return {"success": True, "message": "没有活跃的事务"}
        self._current_backup_id = None
...
class TimeoutHandler:
    ...
    def __init__(self, max_retries: int = 2, default_timeout: int = 120, 
                 backoff_multiplier: float = 1.5, max_backoff: float = 300):
    ...
    def execute_with_retry(self, func: callable, max_retries: int = None, 
                           timeout: int = None, backoff: float = None,
                           on_timeout_callback: callable = None) -> Dict:
        """v4.10: 带重试和退避的超时执行"""
        max_retries = max_retries or self.max_retries
        timeout = timeout or self.default_timeout
        backoff = backoff or self.backoff_multiplier
        ...
        for attempt in range(max_retries + 1):
            attempts += 1
            current_timeout = int(min(timeout * (backoff ** attempt), self.max_backoff))
            ...
            if attempt < max_retries:
                sleep_time = min(2 ** attempt * backoff, self.max_backoff)
        ...
        return {
            ...
            "total_time": time.time() - start_time,
            "skipped": self.should_skip(timeout_count)
        }
    ...
    def execute_with_fallback(self, func: callable, fallback_func: callable = None,
        ...
        result = self.execute_with_retry(func, timeout=timeout, max_retries=max_retries)
    ...
    def get_timeout_stats(self) -> Dict:
        ...
        if not self.timeout_history:
            return {"total": 0, "avg_timeout": 0, "max_timeout": 0}
        
        timeouts = [h.get("timeout_seconds", 0) for h in self.timeout_history]
        return {
            "total": len(self.timeout_history),
...
class ErrorHelper:
    ...
    ERROR_REASON_MAP = {
        "SyntaxError": "代码存在语法错误，无法被 Python 解析",
        "IndentationError": "代码缩进不一致，请统一使用 4 空格",
        "NameError": "使用了未定义的变量或函数名",
        "ModuleNotFoundError": "缺少必要的 Python 包，请运行 pip install",
        "ImportError": "模块导入失败，请检查模块路径和依赖",
        "TypeError": "数据类型不匹配，请检查变量类型",
        "ValueError": "值不符合预期，请检查输入数据格式",
        "AttributeError": "对象没有该属性或方法",
        "KeyError": "字典中不存在指定的键",
        "IndexError": "列表索引超出范围",
        "FileNotFoundError": "文件路径不存在，请检查路径是否正确",
        "PermissionError": "没有文件访问权限",
        "ConnectionError": "网络连接失败，请检查网络和目标地址",
        "TimeoutError": "请求超时，请增加超时时间或检查网络",
        "aider failed": "Aider 执行失败，请检查配置和网络",
        "rate limit": "API 调用频率超限，请等待后重试",
        "TIMEOUT": "任务执行超过预设时间限制",
        "empty output": "任务执行没有产生有效输出",
        "no changes": "代码修改未产生任何变更",
    }
    
    @classmethod
    def get_error_reason(cls, error_output: str) -> str:
        """v4.10: 根据错误输出获取精确的错误原因描述"""
        if not error_output:
            return "未知错误"
        ...
        # 检查自定义错误
        for key, reason in cls.ERROR_REASON_MAP.items():
        ...
        for error_type in python_errors:
            if error_type in error_output:
                return cls.ERROR_REASON_MAP.get(error_type, error_type + " 错误")
        
        return "未知错误类型"
    
    @classmethod
    def get_error_severity(cls, error_output: str) -> str:
        """v4.10: 评估错误严重程度"""
        critical = ["SyntaxError", "IndentationError", "ModuleNotFoundError"]
        high = ["TypeError", "ValueError", "ImportError"]
        medium = ["FileNotFoundError", "TimeoutError", "ConnectionError"]
    ...
    @classmethod
    def get_quick_fix(cls, error_output: str) -> str:
        """v4.10: 获取快速修复建议"""
        reason = cls.get_error_reason(error_output)
        
        quick_fixes = {
            "代码存在语法错误，无法被 Python 解析": "运行 python3 -m py_compile <file> 检查语法",
            "缺少必要的 Python 包，请运行 pip install": "运行 pip install -r requirements.txt",
            "数据类型不匹配，请检查变量类型": "添加类型检查或使用 .get() 方法",
            "文件路径不存在，请检查路径是否正确": "检查文件路径是否正确",
            "任务执行超过预设时间限制": "增加超时时间或简化任务",
        }
        
        return quick_fixes.get(reason, "请查看详细错误信息并根据提示修复")
    
    @classmethod
    def format_error_for_log(cls, error_output: str, context: Dict = None) -> str:
        """v4.10: 为日志格式化的错误信息"""
        reason = cls.get_error_reason(error_output)
        severity = cls.get_error_severity(error_output)
        
        severity_icons = {"critical": "CRITICAL", "high": "HIGH", "medium": "MEDIUM", "low": "LOW"}
        
        lines = [f"[{severity_icons.get(severity, '?')}] {reason}"]
        
        if context:
            if context.get('task'):
                lines.append(f"   Task: {context['task'][:60]}...")
        
        fix = cls.get_quick_fix(error_output)
        if fix:
