...
class SuggestionApplicationService:
    ...
    def _ensure_before(self, action: str, *, subject_id: str = "", execution_id: str = "", payload: Optional[Dict[str, Any]] = None, metadata: Optional[Dict[str, Any]] = None) -> tuple[HookContext, Dict[str, Any] | None]:
        context = self._hook_context(action, subject_id=subject_id, execution_id=execution_id, payload=payload, metadata=metadata)
        ...
        if any((not r.ok or r.blocked) for r in results):
            ...
            return context, {"success": False, "error": result.message if result and result.message else "blocked by hook", "hook": [r.to_dict() for r in results]}
        return context, None
    ...
    async def suggestion_review(self, suggestion_id: str) -> Dict[str, Any]:
        ...
        self._hooks.after("suggestion", SUGGESTION_REVIEW[1], self._hook_context(SUGGESTION_REVIEW[1], subject_id=suggestion_id, payload=context.to_dict()))
    ...
    async def review_suggestion(self, execution_id: str, suggestion_id: str, reviewer: str = "", notes: str = "") -> Dict[str, Any]:
        ...
        _, blocked = self._ensure_before("review_suggestion", subject_id=suggestion_id, execution_id=execution_id, payload={"reviewer": reviewer, "notes": notes})
        if blocked is not None:
            return blocked
        record = await self.governance.review_suggestion(execution_id, suggestion_id, reviewer=reviewer, notes=notes)
        self._hooks.after("suggestion", "review_suggestion", self._hook_context("review_suggestion", subject_id=suggestion_id, execution_id=execution_id, payload={"record": record}))
    ...
    async def approve_suggestion(self, execution_id: str, suggestion_id: str, approver: str = "", notes: str = "") -> Dict[str, Any]:
        if self.governance is None:
            return {"success": False, "error": "Governance is disabled"}
        _, blocked = self._ensure_before(SUGGESTION_APPROVE_SUGGESTION[1], subject_id=suggestion_id, execution_id=execution_id, payload={"approver": approver, "notes": notes})
        if blocked is not None:
            return blocked
        record = await self.governance.approve_suggestion(execution_id, suggestion_id, approver=approver, notes=notes)
        self._hooks.after("suggestion", SUGGESTION_APPROVE_SUGGESTION[1], self._hook_context(SUGGESTION_APPROVE_SUGGESTION[1], subject_id=suggestion_id, execution_id=execution_id, payload={"record": record}))
    ...
    async def reject_suggestion(self, execution_id: str, suggestion_id: str, rejected_by: str = "", notes: str = "") -> Dict[str, Any]:
        if self.governance is None:
            return {"success": False, "error": "Governance is disabled"}
        _, blocked = self._ensure_before("reject_suggestion", subject_id=suggestion_id, execution_id=execution_id, payload={"rejected_by": rejected_by, "notes": notes})
        if blocked is not None:
            return blocked
        record = await self.governance.reject_suggestion(execution_id, suggestion_id, rejected_by=rejected_by, notes=notes)
        self._hooks.after("suggestion", "reject_suggestion", self._hook_context("reject_suggestion", subject_id=suggestion_id, execution_id=execution_id, payload={"record": record}))
    ...
    async def promote_suggestion_to_hitl(
        ...
        *,
        ...
        if self.governance is None:
            return {"success": False, "error": "Governance is disabled"}
        _, blocked = self._ensure_before(SUGGESTION_PROMOTE_TO_HITL[1], subject_id=suggestion_id, payload={"gate": gate, "title": title, "summary": summary, "context": dict(context or {})})
        if blocked is not None:
            return blocked
        ...
        self._hooks.after("suggestion", SUGGESTION_PROMOTE_TO_HITL[1], self._hook_context(SUGGESTION_PROMOTE_TO_HITL[1], subject_id=suggestion_id, payload={"request": request}))
    ...
    async def attach_suggestion_replay(self, suggestion_id: str, replay: Dict[str, Any]) -> Dict[str, Any]:
        if self.governance is None:
            return {"success": False, "error": "Governance is disabled"}
        _, blocked = self._ensure_before(SUGGESTION_ATTACH_REPLAY[1], subject_id=suggestion_id, payload={"replay": dict(replay or {})})
        if blocked is not None:
            return blocked
        request = await self.governance.attach_suggestion_replay(suggestion_id, replay)
        self._hooks.after("suggestion", SUGGESTION_ATTACH_REPLAY[1], self._hook_context(SUGGESTION_ATTACH_REPLAY[1], subject_id=suggestion_id, payload={"request": request}))
        return {"success": True, "data": request}

    async def suggestion_approve(self, suggestion_id: str, approver: str, notes: str = "") -> Dict[str, Any]:
        hook_ctx, blocked = self._ensure_before(SUGGESTION_APPROVE_SUGGESTION[1], subject_id=suggestion_id, payload={"approver": approver, "notes": notes})
        if blocked is not None:
            return blocked
    ...
    async def suggestion_reject(self, suggestion_id: str, approver: str, notes: str = "") -> Dict[str, Any]:
        ...
        self._hooks.after("suggestion", SUGGESTION_REJECT_SUGGESTION[1], self._hook_context(SUGGESTION_REJECT_SUGGESTION[1], subject_id=suggestion_id, payload={"record": record.to_dict()}))
    ...
    async def suggestion_archive(self, suggestion_id: str) -> Dict[str, Any]:
        ...
        self._hooks.after("suggestion", SUGGESTION_ARCHIVE[1], self._hook_context(SUGGESTION_ARCHIVE[1], subject_id=suggestion_id))
