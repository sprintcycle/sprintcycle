...
from ..evolution.models import EvolutionTarget, VersionArtifact
...
class SQLiteVersionRegistry(VersionRegistry):
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS versions (
                    ...
                    source_evolution_request_id TEXT,
                    rollback_to TEXT,
                    promotion_guard_json TEXT,
                    metadata_json TEXT,
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_versions_source_suggestion ON versions(source_suggestion_id)")
            conn.commit()
        ...
        async with self._lock:
            with self._connect() as conn:
                conn.execute(
                    ...
                    (version_id, target, commit_hash, tag, branch, manifest_path, sandbox_id, source_suggestion_id, source_evolution_request_id, rollback_to, promotion_guard_json, metadata_json, is_active)
                    ...
                    (
                        ...
                        artifact.sandbox_id,
                        artifact.source_suggestion_id,
                        artifact.source_evolution_request_id,
                        artifact.rollback_to,
                        json.dumps(artifact.promotion_guard, ensure_ascii=False),
    ...
    def _row_to_artifact(self, row: sqlite3.Row) -> VersionArtifact:
        return VersionArtifact(
            ...
            source_evolution_request_id=row["source_evolution_request_id"],
            rollback_to=row["rollback_to"],
            promotion_guard=json.loads(row["promotion_guard_json"] or "{}"),
