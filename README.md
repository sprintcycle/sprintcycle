...
- 契约构建与汇总主要由 `application/services/lifecycle/lifecycle_contracts.py`、`application/services/lifecycle/lifecycle_contract_assembly_service.py` 等服务完成
...
- 版本与演化能力主要由 `application/services/lifecycle/lifecycle_evolution_service.py`、`application/services/evolution/evolution_version_service.py` 和 `infrastructure/adapters/core/evolution/version_store/` 提供
...
- 这部分逻辑主要分布在 `domain/core/execution/agents/skills.py`、`domain/core/execution/hooks/skill_hooks.py`
...
| **execution** | 执行引擎与任务编排，Sprint 运行时核心 | `core/execution/`（含 agents、core、hooks、orchestrator） |
| **evolution** | 版本演化与晋升，versioned evolution 能力 | `core/evolution/` |
| **governance** | 治理与建议处理，HITL 审查与门禁 | `core/governance/`（含 quality_spec、hitl、suggestion） |
...
| **ports** | 基础设施端口抽象（依赖注入工厂注册） | `generic/ports/` |
...
- 位于 `domain/core/execution/agents/`
...
- `docs/ARCHITECTURE_INVARIANTS.md` — 架构不变性文档
