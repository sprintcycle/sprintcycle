...
|---------|----------|----------------------|
| **A — Import / path** | `ModuleNotFoundError`, `ImportError`, pytest collect-only, `lint-imports` import graph | **Path migration first** — see § Path migration below |
| **B — Architecture contract** | `lint-imports` forbidden layer | Move code or imports across layers per `sprintcycle-architecture-orchestration.mdc`; never silence contracts |
