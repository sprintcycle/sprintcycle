...
**确认流程：**
1. **AI 生成 PRD** → 等待用户审查
2. **用户反馈与确认** → 用户确认 PRD 或提出修改意见
3. **PRD 调整** → 根据用户反馈调整 PRD
4. **最终确认** → 用户最终确认 PRD 后进入 Phase 2 方案设计阶段
...
**使用工具：** PRD 生成后，使用 `NotifyUser` 或 `AskUserQuestion` 工具让用户确认和修改，收到用户确认前**禁止**进入 Phase 2 方案设计阶段。
